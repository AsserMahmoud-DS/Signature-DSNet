...
"""
DetailSemNet
Some implementations are modified from timm (https://github.com/rwightman/pytorch-image-models) & iformer(https://github.com/sail-sg/iFormer).
"""
import math
import logging
...
from .my_module import HighMixer_v2, HighMixer_v3
...
def haar_highpass_to_fullres(lh, hl, hh):
    top_left = (lh + hl + hh) * 0.5
    top_right = (-lh + hl - hh) * 0.5
    bottom_left = (lh - hl - hh) * 0.5
    bottom_right = (-lh - hl + hh) * 0.5

    b, c, h, w = lh.shape
    out = lh.new_zeros(b, c, h * 2, w * 2)
...
class Mixer(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., attention_head=1, pool_size=2, 
        **kwargs, ):
        ...
        elif HighMixer_flag == 2:
            self.high_mixer = HighMixer_v2(high_dim)
        elif HighMixer_flag == 3:
            self.high_mixer = HighMixer_v3(high_dim)
    ...
    def devide_freq_v2(self, x):
        
        fft_result = torch.fft.rfft2(x)

        # Compute magnitude spectrum
        magnitude_spectrum = torch.abs(fft_result)
        
        # Set a threshold (e.g., top 10%)
        threshold = torch.quantile(magnitude_spectrum, 0.8)
        
        # Create masks for high and low frequencies
        high_freq_mask = magnitude_spectrum > threshold
        low_freq_mask = ~high_freq_mask

        # Apply masks to FFT result
        high_freq_components = fft_result * high_freq_mask
        low_freq_components = fft_result * low_freq_mask

        # Compute inverse FFT to obtain time-domain signals
        high_freq_signal = torch.fft.irfft2(high_freq_components)
        low_freq_signal = torch.fft.irfft2(low_freq_components)
        high_freq_signal = self.reduce_h(high_freq_signal)
        low_freq_signal = self.reduce_l(low_freq_signal)
        low_freq_signal = self.pool(low_freq_signal)
        return high_freq_signal, low_freq_signal

    def devide_freq_v3(self, x):
        # B, C, H, W
        # downsample
        if self.pool_size > 1:
            lx = self.pool(x)
    ...
    def forward(self, x):
        ...
        if self.devide_module_type == 'v1':
            hx, lx = self.devide_freq(x)
        elif self.devide_module_type == 'v2':
            if H%2 == 1: # last stage
                hx, lx = self.devide_freq(x) # original iformer module
            else:
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            output, hx, lx = self.attn(self.norm1(x))
            x = x + self.drop_path(self.layer_scale_1 * output)
...
@register_model
def dsnet(pretrained=False, **kwargs):
    """
